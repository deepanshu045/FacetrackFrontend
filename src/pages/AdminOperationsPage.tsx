...
export default function AdminOperationsPage({ mode }: Props) {
  ...
  return <PageWrap>
    ...
    {mode === "class-sections" && <><form onSubmit={submitSection} className="mb-6 grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-5 md:grid-cols-4"><Input label="Department" value={sectionForm.department} onChange={e => setSectionForm({ ...sectionForm, department: e.target.value })} disabled={submitting} /><div><label className="mb-1.5 block text-xs font-medium text-slate-400">Class</label><select value={sectionForm.class_name} onChange={e => setSectionForm({ ...sectionForm, class_name: e.target.value })} disabled={submitting} className="w-full rounded-xl border border-white/10 bg-[#0F172A] px-3 py-2.5 text-sm text-white disabled:opacity-50"><option value="">Select class</option>{CLASS_OPTIONS.map(value => <option key={value} value={value}>{value}</option>)}</select></div><Input label="Section" value={sectionForm.section} onChange={e => setSectionForm({ ...sectionForm, section: e.target.value })} disabled={submitting} /><Button type="submit" className="mt-auto" disabled={submitting}>{submitting ? <><Loader2 size={16} className="animate-spin" /> Creating…</> : <><Plus size={16} /> Create</>}</Button></form><List rows={sections.map(s => ({ id: s.id, text: `${s.department} · ${s.class_name} · ${s.section}`, onDelete: () => deleteClassSection(s.id) }))} reload={load} /></>}
