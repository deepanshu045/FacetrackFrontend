...
export default function AdminOperationsPage({ mode }: Props) {
  ...
  return <PageWrap>
    ...
    {mode === "teachers" && <><form onSubmit={submitTeacher} className="mb-6 grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-5 md:grid-cols-4"><Input label="Username" value={teacherForm.username} onChange={e => setTeacherForm({ ...teacherForm, username: e.target.value })} disabled={submitting} /><Input label="Name" value={teacherForm.name} onChange={e => setTeacherForm({ ...teacherForm, name: e.target.value })} disabled={submitting} /><Input label="Email" type="email" value={teacherForm.email} onChange={e => setTeacherForm({ ...teacherForm, email: e.target.value })} disabled={submitting} /><Input label="Password" type="password" value={teacherForm.password} onChange={e => setTeacherForm({ ...teacherForm, password: e.target.value })} disabled={submitting} /><Button type="submit" disabled={submitting}>{submitting ? <><Loader2 size={16} className="animate-spin" /> Creating…</> : <><Plus size={16} /> Create Teacher</>}</Button></form><form onSubmit={submitAssignment} className="mb-6 grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-5 md:grid-cols-3"><select disabled={submitting} className="rounded-xl border border-white/10 bg-[#0F172A] px-3 py-2.5 text-sm text-white disabled:opacity-50" value={assignment.teacherId} onChange={e => setAssignment({ ...assignment, teacherId: e.target.value })}><option value="">Select teacher</option>{teachers.map(t => <option key={t.id} value={t.id}>{t.name} ({t.username})</option>)}</select><select disabled={submitting} className="rounded-xl border border-white/10 bg-[#0F172A] px-3 py-2.5 text-sm text-white disabled:opacity-50" value={assignment.classSectionId} onChange={e => setAssignment({ ...assignment, classSectionId: e.target.value })}><option value="">Select class</option>{sections.map(s => <option key={s.id} value={s.id}>{sectionName(s.id)}</option>)}</select><Button type="submit" disabled={submitting}>{submitting ? <><Loader2 size={16} className="animate-spin" /> Assigning…</> : "Assign Class"}</Button></form><List rows={teachers.map(t => ({ id: t.id, text: `${t.name} · ${t.username} · ${t.email || "no email"}`, onDelete: undefined }))} reload={load} /></>}
